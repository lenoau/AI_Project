...
def predict(events: List[EventRecord]):
    ...
    le_event.fit(df['event_type'])  # 학습

    le_hub = LabelEncoder()
    le_hub.fit(df['hub_type'])  # 학습
    ...
    try:
        ...
        print("Warning: MinMaxScaler 파일 없음. 새로 학습합니다.")
        scaler = MinMaxScaler()
        scaler.fit(df[['time_delta']])
    ...
    try:
        ...
        print("Warning: MinMaxScaler 재학습 수행")
        scaler.fit(df[['time_delta']])
        df['time_delta_scaled'] = scaler.transform(df[['time_delta']])
